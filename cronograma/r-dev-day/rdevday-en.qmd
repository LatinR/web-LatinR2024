...
-   Proposing improvements to the R help files (that are accessed via `help` or `?` in R)
-   Analysing a reported bug in R
-   Developing a fix to a reported bug in R
-   Adding content to the [R Dev Guide](https://contributor.r-project.org/rdevguide/), the guide for contributors to base R
-   Improving the [R Translations Dashboard](https://contributor.r-project.org/translations-dashboard/), the dashboard used to monitor the status of translations
...
| UTC           | Buenos Aires and Brasília (UTC-3) | Hermosillo (UTC-7) |
|:--------------|:---------------------|:-------------------|
...
We will use the "r-dev-day" channel on the [LatinR Slack](https://join.slack.com/t/latin-r/shared_invite/zt-2tespojdh-OpXaVcnzOY_SETtv9hqRSQ) for related discussion - use of English, Spanish or Portuguese is welcome.
...
Add your name to the list of contributors if you are interested to work on an issue.
You can add yourself to multiple issues as it is not a final commitment - allocations will be finalised at the start of each session.

Please keep an eye on the issues in the run up to the event.
You may need additional preparation to work on certain issues, e.g. learning to build R on macOS or reading background material.

## Development setup

Most people will be able to work through the browser only (using Weblate, GitHub browser interface, GitHub Codespaces or Gitpod).
Work on OS-specific bugs may require using or building R on your own computer.
...
The [Further Resources](#further-resources) section contains links for learning more about contribution.
Working through these in advance is entirely optional - we will aim to match you to tasks given your current knowledge and experience - but they will help you to get stuck in at the event.
